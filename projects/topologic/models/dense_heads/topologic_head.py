...
class TopoLogicHead(AnchorFreeHead):
    ...
    def _init_layers(self):
        ...
        # 차선 분류 하기 위한 신경망 가지 (Lane Classification)
        # 여기서 분류란 모델이 예측한 많은 Query들 중에서 이것이 진짜 차선인가? 아니면 배경인가 판단하는 것이다.
        # num_ref_fcs : 중간에 몇개의 FC layer 층을 쌓을 것인가 결정하는 하이퍼 파라미터 여기서는 2를 사용
        # 논문에서는 3개 MLP을 사용한다고 나와있다.
        cls_branch = []
        ...
        # 이 신경망은 출력으로 차선일 확률이 나온다.

        # Lane Regression을 위한 신경망
        # 회귀란 모델이 이것은 진짜 차선이다 라고 판단한 쿼리에 대해 그렇다면 그 차선의 정확한 3D 좌표는 어떤것인가 11x3형태이다.
        # code_size = 3*11 = 33
        reg_branch = []
        for _ in range(self.num_reg_fcs):
        ...
        # Embeding Network : TE 특징을 -> LC 특징과 결합하기 좋게 변환하고 강화하는 부분.
        te_embed_branch = []
