...
class TopoLogic(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        # lane_head -> pts_bbox_head 의 이름으로 사용됨
    ...
    def extract_img_feat(self, img, img_metas, len_queue=None):
        ...
        """img_backbone, img_neck을 사용하여 이미지 특징을 추출하고, 다중 카메라 입력에 맞게 형태를 변환한다."""
        B = img.size(0)
        if img is not None:
    ...
    def extract_feat(self, img, img_metas=None, len_queue=None):
        """extract_img_feature을 호출하여 이미지 특징을 추출하는 Wrapper 함수입니다."""
    ...
    def obtain_history_bev(self, imgs_queue, img_metas_list):
        ...
        """
        """비디오 시퀀스에서 이전 프레임들의 BEV 특징을 순차적으로 계산하여 현재 프레임의 BEV 특징 생성에 활용합니다."""
        self.eval()
    ...
    def forward_train(self,
        ...
        prev_img = img[:, :-1, ...] #(1,0,7,3,800,1024)
        img = img[:, -1, ...] # (1,7,3,800,1024)
        ...
        if self.video_test_mode:
            ...
            prev_bev = None # 이전 t BEV는 사용하지 않음. 실제로 이게 실행됨.
        ...
        # TE Detection 감지에 대한 손실을 저장할 딕셔너리
        te_losses = {}
        # 예측을 실행 bbox_out 안에는 바운딩 박스 위치, 각 박스가 특정 객체일 확률의 정보가 들어있다.
        bbox_outs = self.bbox_head(front_view_img_feats, bbox_img_metas)
        # 손실값(e.x {'loss_cls' : 0.5, 'loss_bbox':1.2})과 bipartite matching의 결과를 뜻하는 것 같다
        bbox_losses, te_assign_result = self.bbox_head.loss(bbox_outs, gt_bboxes, gt_labels, bbox_img_metas, gt_bboxes_ignore)

        #bbox_head가 예측 과정에서 만들어낸 유용한 중간 결과물들을 다음 (차선 감지)에서 사용하기 위해 추출함.
        # 감지된 각 객체에 대한 풍부한 feature vector를 가져온다. 이 특징은 단순히 위치나 종류 뿐만
        # 아니라 해당 객체의 더 깊은 semantic information을 담고 있다
        te_feats = bbox_outs['history_states']
        te_cls_scores = bbox_outs['all_cls_scores']
        # 위에 2개의 정보는 차선감지 헤드에서  "주변에 이런 객체들이 있으니 참고해서 차선을 더 잘 찾아봐"
        # Scene Knowledge Graph에서 사용됨. 논문 3.3.4


        # 나중에 lane_head의 손실과 이름이 겹치치 않게 하기 위해 사용됨.
        for loss in bbox_losses:
            te_losses['bbox_head.' + loss] = bbox_losses[loss]

        # 예외 처리 현재 이미지에 정답 객체가 하나도 없는 경
